...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, **kwargs):
        ...
        if self.same_focals:
            self.im_focals = nn.Parameter(
                torch.FloatTensor(
                    [[torch.tensor(self.focal_break) * np.log(max(self.imshapes[0]))]]
                )
            )  # initialize with H x W of first image
        else:
    ...
    def depth_to_pts3d(self):
        ...
        rel_ptmaps = _fast_depthmap_to_pts3d(
            depth, self._grid, focals, pp=pp, same_focals=self.same_focals
        )
