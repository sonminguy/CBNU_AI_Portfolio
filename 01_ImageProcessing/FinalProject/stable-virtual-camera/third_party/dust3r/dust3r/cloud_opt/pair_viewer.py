...
from dust3r.utils.geometry import depthmap_to_absolute_camera_coordinates, geotrf, inv
...
class PairViewer(BasePCOptimizer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.same_focals:
            self.focals = nn.Parameter(
                torch.tensor([torch.tensor(self.focals).mean()]), requires_grad=False
            )
        else:
    ...
    def depth_to_pts3d(self):
        ...
        for i, (d, im_pose) in enumerate(zip(self.depth, self.get_im_poses())):
            ...
            pts, _ = depthmap_to_absolute_camera_coordinates(
                d.cpu().numpy(), intrinsic.cpu().numpy(), im_pose.cpu().numpy()
            )
