...
class HabitatEnvironmentMapRenderer:
    def __init__(
        ...
        self.equirectangular_projection = projections.EquirectangularProjection(
            *equirectangular_resolution
        )
        # 3D unit ray associated to each pixel of the equirectangular map
        equirectangular_rays = projections.get_projection_rays(
            self.equirectangular_projection
        )
        # Not needed, but just in case.
        equirectangular_rays /= np.linalg.norm(
            equirectangular_rays, axis=-1, keepdims=True
        )
        ...
        self.equirectangular_depth_scale_factors = 1.0 / np.max(
            np.abs(equirectangular_rays), axis=-1
        )
        ...
        if self.seed == None:
            ...
            if self.render_cubemap:
                for face_id, orientation in enumerate(CUBEMAP_FACE_ORIENTATIONS_ROTVEC):
                    ...
                    rgb_sensor_spec.uuid = (
                        f"color_cubemap_{CUBEMAP_FACE_LABELS[face_id]}"
                    )
                    ...
                    if self.render_depth:
                        ...
                        depth_sensor_spec.uuid = (
                            f"depth_cubemap_{CUBEMAP_FACE_LABELS[face_id]}"
                        )
    ...
    def render_viewpoint(self, viewpoint_position):
        ...
        try:
            ...
            viewpoint_observations[
                "depth_equirectangular"
            ] *= self.equirectangular_depth_scale_factors
        except KeyError:
            pass
